...
## Phase 4 使用指南：确定性评测 Benchmark

Phase 4 在 Phase 3.5 评测 MVP 基础上，建立正式的 benchmark JSONL schema 和多类型确定性检查。所有 benchmark 数据为小型人工/确定性 fixture，不提交真实 PDF 或完整候选生成物。

**Schema 定义**：[`eval/qa/schema.md`](eval/qa/schema.md) 定义 7 种检查类型，每条 check 包含 `id`、`type`、`category`、`severity`、`expected`、`notes` 字段。

### Benchmark 文件

| 文件 | 类型 | 说明 |
|------|------|------|
| `eval/qa/phase4_smoke.jsonl` | 冒烟测试 | 覆盖全部 7 种 check type 的基础验证 |
| `eval/qa/gb-7258-2017_factq.jsonl` | 事实查询 | 文档元数据、条款标签、关键词召回、evidence 引用 |
| `eval/qa/gb-7258-2017_dateq.jsonl` | 日期查询 | 实施日期、发布日期、过渡期等时间相关断言 |
| `eval/qa/gb-7258-2017_citationq.jsonl` | 引用查询 | 条款 evidence quote 存在性和内容匹配 |

### 评测命令

运行全部 Phase 4 benchmark：

```bash
.venv/bin/python tools/eval_candidates.py eval/qa/phase4_smoke.jsonl eval/qa/gb-7258-2017_factq.jsonl eval/qa/gb-7258-2017_dateq.jsonl eval/qa/gb-7258-2017_citationq.jsonl --document-id gb-7258-2017
```

单独运行某一类型：

```bash
.venv/bin/python tools/eval_candidates.py eval/qa/gb-7258-2017_factq.jsonl --document-id gb-7258-2017
```
...
| Type | 断言内容 | Failure Category |
|------|---------|-----------------|
| `document_id_exists` | 文档 ID 存在 | `missed_document` |
| `provision_label_exists` | 条款标签存在 | `missed_provision` |
| `keyword_search` | 关键词检索命中数 | `missed_provision` |
| `requirement_modality_exists` | 要求情态存在 | `missed_requirement` |
| `evidence_quote_exists` | 引用原文包含子串 | `citation_missing` |
| `metadata_field_equals` | 元数据字段精确匹配 | `metadata_mismatch` |
| `topic_tag_exists` | 主题标签存在 | `topic_mismatch` |
...
Vector index
FastAPI QA service
Answer-engine grading / LLM rubric（Phase 4 后续增强）
批处理任务队列
多模型审核工作流
```
